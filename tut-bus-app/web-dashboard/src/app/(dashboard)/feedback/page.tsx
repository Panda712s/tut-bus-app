  ...
  return (
    <span
      className={`inline-flex items-center whitespace-nowrap rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${
        negative ? 'bg-red-50 text-red-700 ring-red-600/20' : 'bg-slate-100 text-slate-600 ring-slate-500/20'
      }`}
    >
    ...
    <div>
      ...
      {ratings.length > 0 && (
        <section className="mb-8">
          <h2 className="mb-3 text-sm font-semibold text-slate-700">Post-trip ratings</h2>
      ...
      <section>
        {ratings.length > 0 && <h2 className="mb-3 text-sm font-semibold text-slate-700">All feedback</h2>}
        <div className="space-y-3">
          {feedback.map((f) => (
